...
from odoo.addons.payment_airwallex import utils as airwallex_utils
from odoo.addons.payment_airwallex.const import PAYMENT_STATUS_MAPPING
...
class PaymentTransaction(models.Model):
    ...
    # See https://developer.airwallex.com/docs/api-basics/notifications/ipn/IPNandPDTVariables/
    # this field has no use in Odoo except for debugging
    airwallex_type = fields.Char(string="Airwallex Transaction Type")

    def _get_specific_processing_values(self, processing_values):
        """ Override of `payment` to return the Airwallex-specific processing values.
            ...
            'purchase_units': [
                {
                    ...
                    'payee':  {
                        ...
                        'email_address': airwallex_utils.get_normalized_email_account(self.provider_id)
                    },
                },
            ],
            'payment_source': {
        ...
        order_data = self.provider_id._airwallex_make_request(
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        """ Override of `payment` to find the transaction based on Airwallex data.
        ...
        if not tx:
            raise ValidationError(
                "Airwallex: " + _("No transaction found matching reference %s.", reference)
            )
        return tx

    def _process_notification_data(self, notification_data):
        """ Override of `payment` to process the transaction based on Airwallex data.
        ...
        assert amount and currency_code, "Airwallex: missing amount or currency"
        assert self.currency_id.compare_amounts(float(amount), self.amount) == 0, \
            "Airwallex: mismatching amounts"
        assert currency_code == self.currency_id.name, "Airwallex: mismatching currency codes"
        ...
        self.airwallex_type = txn_type

        # Force Airwallex as the payment method if it exists.
        self.payment_method_id = self.env['payment.method'].search(
            [('code', '=', 'airwallex')], limit=1
        ...
        if payment_status in PAYMENT_STATUS_MAPPING['pending']:
            ...
            self._set_error(
                "Airwallex: " + _("Received data with invalid payment status: %s", payment_status)
            )
